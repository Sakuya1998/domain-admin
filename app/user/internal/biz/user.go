...
import (
	...
	errorsv1 "github.com/Sakuya1998/domain-admin/api/errors/v1"
...
func (uc *UserUsecase) Register(ctx context.Context, req *pb.RegisterRequest) (*pb.RegisterReply, error) {
	...
	if err == nil && existingUser != nil {
		return nil, errorsv1.ErrorUserAlreadyExists("user already exists")
	}

	// Check if email already exists
	existingUser, err = uc.repo.FindByEmail(ctx, req.Email)
	if err == nil && existingUser != nil {
		return nil, errorsv1.ErrorEmailAlreadyExists("email already exists")
	}
...
func (uc *UserUsecase) Login(ctx context.Context, req *pb.LoginRequest) (*pb.LoginReply, error) {
	...
	if req.GetUsername() != "" {
		...
		return nil, errorsv1.ErrorUserNotFound("username or email is required")
	}

	if err != nil {
		return nil, errorsv1.ErrorUserNotFound("user not found")
	}

	// In real implementation, verify hashed password
	if user.Password != req.Password {
		return nil, errorsv1.ErrorInvalidCredentials("invalid credentials")
	}

	if user.Status != commonv1.Status_STATUS_ACTIVE {
		return nil, errorsv1.ErrorUserInactive("user is inactive")
	}
...
func (uc *UserUsecase) UpdateUserPassword(ctx context.Context, req *pb.UpdateUserPasswordRequest) error {
	...
	user, err := uc.repo.FindByID(ctx, req.Id)
	if err != nil {
		return errorsv1.ErrorUserNotFound("user not found")
	}

	// In real implementation, verify old password and hash new password
	if user.Password != req.OldPassword {
		return errorsv1.ErrorInvalidCredentials("invalid old password")
	}
